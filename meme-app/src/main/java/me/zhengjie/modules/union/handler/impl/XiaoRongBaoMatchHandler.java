...
import me.zhengjie.modules.util.ShanQianAESUtils;
 ...
 */
@Slf4j
@LoanProductMatchHandler(name = "xiaoRongBao-matcher", confKey = "xiaoRongBao")
public class XiaoRongBaoMatchHandler extends AbstractProductDbMatchHandler<JSONObject> {

    private static final String SUCCESS_CODE = "0000";
    private static final String LOG_LEVEL = "小融宝";
...
//        paramMap.put("channelSign", product.getChaSign());
    ...
    protected Map<String, Object> buildRequestParamRegister(Product product, HxUser user, BaseLoanProductConfigProperties configProperties) {
        Map<String, Object> paramMap = new HashMap<>();
        try {
            paramMap.put("phone", ShanQianAESUtils.encrypt(user.getPhone(), configProperties.getAppSecret(), configProperties.getAppIv()));
            paramMap.put("sign", configProperties.getAppKey());
        } catch (Exception e) {
            log.error("{}加密异常", LOG_LEVEL, e);
            e.printStackTrace();
            return null;
        }
