...
@Slf4j
public class LoanChannelMatchHandlerHolder {

    public static final String SUFFIX = "-matcher";

    private static final Map<String, AbstractChannelDbMatchHandler<?>> HANDLER_MAP = Maps.newConcurrentMap();


    public static void register(AbstractChannelDbMatchHandler<?> handler) {
        log.info("register handler : {}", handler.getClass().getSimpleName());
        LoanChannelMatchHandler annotation = handler.getClass().getAnnotation(LoanChannelMatchHandler.class);
        if (Objects.isNull(annotation)) {
            log.warn("{}该类未被[LoanChannelMatchHandler]注解标注,无法注册", handler.getClass());
            throw new IllegalArgumentException("请标注[LoanChannelMatchHandler]注解");
        }
    ...
    public static AbstractChannelDbMatchHandler<?> getHandler(String code) {
        if (CharSequenceUtil.isBlank(code)) {
            return null;
        }
        String handlerCode = code + SUFFIX;
        AbstractChannelDbMatchHandler<?> abstractApiProductMatchHandler = HANDLER_MAP.get(handlerCode);
